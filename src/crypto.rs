//! Session crypto: XChaCha20-Poly1305  +  HKDF-SHA-256 (no extra crates).
...
/* ---------- minimalist HKDF-SHA-256 ---------- */
fn hkdf_sha256(ikm: &[u8], info: &[u8]) -> [u8; 32] {
    let salt = [0u8; 32];
    let prk  = hmac_sha256(&salt, ikm);
    let mut t_in = Vec::with_capacity(info.len() + 1);
    t_in.extend_from_slice(info);
    t_in.push(0x01);
    hmac_sha256(&prk, &t_in)
}
...
fn hmac_sha256(key: &[u8], data: &[u8]) -> [u8; 32] {
    ...
    outer.extend_from_slice(&opad); outer.extend_from_slice(&inner_hash);
    sha256(&outer)
}

/* ---------- tiny, self-contained SHA-256 ---------- */
